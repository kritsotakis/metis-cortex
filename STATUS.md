...
**Last updated:** 2026-05-09 ~21:05 AEST · Claude Code (Stripe products built — 3 SKUs live in shared trust Stripe account `acct_1Qi7qr2nwvvosadL`: Setup Fee A$1,500 one-time, AI Receptionist Standard A$1,200/mo, AI Receptionist Founding A$800/mo. Following the cross-business brand convention — full convention at `~/.claude/memory/stripe-brand-convention.md`.)
**Prior update:** 2026-05-09 ~18:30 AEST · Claude Code (🟢 SITE LIVE — claude.ai/design 18-finding remediation brief executed. P1 + P3 + most P2 shipped to working tree (19 files): ink token repainted #0e1e2e → #0f203f to match official lockup PNG navy (no more visible plate behind logo); gold token codemod-renamed to bronze across all components (alias kept in globals.css for safety); inline JSX wordmark replaced by official horizontal-lockup PNG in header (40px) and stacked-lockup-on-light PNG in footer (96px) and horizontal-lockup-on-light in /privacy header; bronze culled to "one accent per section" rule with guarantee section as the loudest moment; placeholder DSKCaseStudy commented out of page.tsx until Day 28 numbers land; italic-bronze reserved for hero only (ClosingCTA "7pm tonight" stripped); CTA inverted to bg-bone-on-ink with bronze on chevron glyph only (→ replaced with ›); rounded-2xl swept to rounded-lg; jump-nav added to header (How it works · Pricing · The guarantees); CTA copy varied (Hero "Book a 15-min demo" / Guarantee "Claim the guarantee" / Closer "Hear Zoe — book the demo"). HELD: phone CTA in hero (Peter approved option a — skip until Zoe deployed on Twilio); email shift to info@metiscortex.au (gated on Cowork email-forward setup). ASIC business name registered 9 May 2026 (no separate reg number — name itself is the identifier). **Working tree at 19 uncommitted files awaiting Peter's `commit` go.**)
...
| 2026-05-09 | Peter + Code | **Metis Cortex Stripe products built** in the shared Kritsotakis Family Trust Stripe account (acct_1Qi7qr2nwvvosadL). 3 SKUs created following the trust-rebrand brand convention (Model B / full per-charge `statement_descriptor`, per-product metadata schema, AUD-only since Metis Cortex is AU-focused). Setup Fee A$1,500 one-time + AI Receptionist Standard A$1,200/mo + AI Receptionist Founding A$800/mo (cap=5, first 5 case-study clients across cleaning/restaurant/RE/dental/beauty). Tier 2 (Knowledge add-on) deferred — needs per-vertical pricing locked before SKU creation. Brand convention doc updated to mark Metis Cortex integration map as built. Reference: `~/.claude/memory/stripe-brand-convention.md`. |
...
| Stripe brand convention (cross-business — single trust account, full per-charge descriptor model, per-product metadata schema) | `~/.claude/memory/stripe-brand-convention.md` (Code only — content also inlined in EXIT CODE PAIR.md 18:10 / 18:25 entries for Cowork visibility) |
| Metis Cortex Stripe products | account `acct_1Qi7qr2nwvvosadL` · Setup Fee `prod_UU7Xw8XuMlkD1W` (price `price_1TV9Jo2nwvvosadLTkBGJXDj` A$1,500) · AI Receptionist `prod_UU7XyC7hIX9jIu` (Standard price `price_1TV9Jw2nwvvosadLXDCrFp1i` A$1,200/mo · Founding price `price_1TV9K12nwvvosadLZcdlSFgC` A$800/mo cap=5) |
